...
def _fmt_price(val):
    try:
        return f"{float(val)/1e9:.2f}B VND"
    except Exception:
...
def _write_sheet(ws, records):
    ...
    for row_idx, r in enumerate(records, 2):
        ...
        try:
            value_b = round(float(r.get("priceInit", 0)) / 1e9, 3)
        except Exception:
            value_b = ""
        ...
        try:
            award_b = round(float(r.get("winner_price", 0) or 0) / 1e9, 3) if r.get("winner_price") else ""
        except Exception:
            award_b = ""
